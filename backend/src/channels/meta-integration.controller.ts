...
export class MetaIntegrationController {
    ...
    async callback(
        ...
        try {
            ...
            return res.redirect(`/integrations/meta/error?reason=generic&details=${encodeURIComponent(error.message)}`);
        }
    }

    private setSecurityHeaders(res: Response) {
        res.setHeader('Content-Security-Policy', "default-src 'self' https://fonts.googleapis.com https://fonts.gstatic.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com;");
