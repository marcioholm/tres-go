...
export class TagsController {
    ...
    @Post()
    create(@Param('workspaceId') workspaceId: string, @Body() data: { name: string, color: string }) {
        return this.tagsService.create(workspaceId, data.name, data.color);
    }

    @Patch(':id')
    update(@Param('workspaceId') workspaceId: string, @Param('id') id: string, @Body() data: any) {
