import { Controller, Get, Patch, Post, Delete, Put, Param, Body, UseGuards } from '@nestjs/common';
...
export class WorkspacesController {
    ...
    @Post('members/invite')
    inviteMember(@Param('workspaceId') workspaceId: string, @Body() body: { email: string, role: string }) {
        return this.workspacesService.inviteMember(workspaceId, body.email, body.role);
    }

    @Patch('members/:userId')
    updateMember(@Param('workspaceId') workspaceId: string, @Param('userId') userId: string, @Body() data: any) {
    ...
    @Put('business-hours')
    updateBusinessHours(@Param('workspaceId') workspaceId: string, @Body() hours: any[]) {
    ...
    @Post('quick-replies')
    createQuickReply(@Param('workspaceId') workspaceId: string, @Body() data: any) {
        return this.workspacesService.createQuickReply(workspaceId, data.shortcut, data.content);
    }

    @Delete('quick-replies/:id')
    deleteQuickReply(@Param('workspaceId') workspaceId: string, @Param('id') id: string) {
