...
export class ContactsService {
    ...
    async updateSource(workspaceId: string, id: string, sourceData: any, userId?: string) {
        const oldContact = await this.prisma.contact.findUnique({ where: { id } });
        const updatedContact = await this.prisma.contact.update({
            where: { id },
            data: {
    ...
    async addNote(workspaceId: string, contactId: string, userId: string, content: string) {
    ...
    async bulkTagAction(workspaceId: string, data: { contactIds: string[], tagId: string, action: 'add' | 'remove' }) {
