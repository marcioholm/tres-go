...
export class ContactsController {
    ...
    @Patch(':id')
    update(@Param('workspaceId') workspaceId: string, @Param('id') id: string, @Body() data: any, @Req() req: any) {
        return this.contactsService.update(workspaceId, id, data, req.user?.sub);
    }

    @Patch(':id/source')
    updateSource(@Param('workspaceId') workspaceId: string, @Param('id') id: string, @Body() data: any, @Req() req: any) {
        return this.contactsService.updateSource(workspaceId, id, data, req.user?.sub);
    }

    @Delete(':id')
    remove(@Param('workspaceId') workspaceId: string, @Param('id') id: string, @Req() req: any) {
    ...
    @Post(':id/notes')
    addNote(@Param('workspaceId') workspaceId: string, @Param('id') id: string, @Body() body: { content: string, userId: string }) {
    ...
    importCsv(@Param('workspaceId') workspaceId: string, @UploadedFile() file: any) {
        return this.contactsService.importCsv(workspaceId, file);
    }

    @Post('bulk-tags')
    bulkTagAction(@Param('workspaceId') workspaceId: string, @Body() data: { contactIds: string[], tagId: string, action: 'add' | 'remove' }) {
