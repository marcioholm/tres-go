...
export class MetricsService {
    ...
    async getAgentMetrics(workspaceId: string, agentId: string, period: any) {
        ...
        return {
            ...
            fulfillment: {
                firstResponse: avgFirstResponse <= (config?.firstResponseGoal || 5) ? 'MET' : 'BEHIND',
                resolution: avgResolution <= (config?.resolutionGoal || 1440) ? 'MET' : 'BEHIND',
                conversion: conversionRate >= (config?.conversionRateGoal || 30) ? 'MET' : 'BEHIND',
            },
        };
    }
    ...
    async registerConversion(
        ...
        } else if (config?.saleAttribution === 'LAST_AGENT' && sessions.length > 0) {
