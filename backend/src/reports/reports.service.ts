...
export class ReportsService {
    ...
    async getDashboardMetrics(workspaceId: string, range: { start: string, end: string }) {
        ...
        const ticketMedia = sales._count > 0 ? Math.round(revenue / sales._count) : 0;
        ...
        const performance = await Promise.all(agents.map(async (awu) => {
            const [count, resolved, sales] = await Promise.all([
                this.prisma.conversation.count({
                    where: { workspaceId, agentId: awu.userId, createdAt: { gte: start } }
                }),
                this.prisma.conversation.count({
                    where: { workspaceId, agentId: awu.userId, status: 'CLOSED', updatedAt: { gte: start } }
                }),
                this.prisma.sale.aggregate({
                    where: { workspaceId, agentId: awu.userId, createdAt: { gte: start } },
                    _sum: { amount: true }
    ...
    async getFunnelMetrics(workspaceId: string) {
        ...
        if (columns.length === 0) {
            ...
            return [
                { name: 'Abertos', color: '#6366f1', value: await this.prisma.conversation.count({ where: { workspaceId, status: 'OPEN' } }) },
                { name: 'Em Atendimento', color: '#f59e0b', value: await this.prisma.conversation.count({ where: { workspaceId, status: 'ACTIVE' } }) },
                { name: 'Fechados', color: '#10b981', value: await this.prisma.conversation.count({ where: { workspaceId, status: 'CLOSED' } }) }
            ];
        }
