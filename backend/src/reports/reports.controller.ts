...
export class ReportsController {
    ...
    @Get('dashboard')
    getDashboardMetrics(@Param('workspaceId') workspaceId: string, @Query('start') start: string, @Query('end') end: string) {
        return this.reportsService.getDashboardMetrics(workspaceId, { start, end });
    }

    @Get('agents')
    getAgentPerformance(@Param('workspaceId') workspaceId: string, @Query('start') start: string, @Query('end') end: string) {
        return this.reportsService.getAgentPerformance(workspaceId, { start, end });
    }

    @Get('volume')
    getVolumeByDay(@Param('workspaceId') workspaceId: string, @Query('start') start: string, @Query('end') end: string) {
