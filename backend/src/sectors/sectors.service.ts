...
export class SectorsService {
  ...
  async create(
    ...
    if (!options.skipLimitCheck) {
      ...
      if (!limitInfo.allowed) {
        throw new Error(
          `Limite de setores (${limitInfo.limit}) atingido para o seu plano.`,
        );
      }
    }
  ...
  async delete(workspaceId: string, sectorId: string) {
    ...
    if (sector._count.conversations > 0) {
      ...
      if (!defaultSector) {
        throw new Error(
          'Cannot delete sector with open conversations without another default sector to migrate to.',
        );
      }
