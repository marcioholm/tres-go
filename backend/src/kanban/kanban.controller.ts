...
export class KanbanController {
    ...
    @Post('deals')
    createDeal(@Param('workspaceId') workspaceId: string, @Body() data: any, @Req() req: any) {
        return this.kanbanService.createDeal(workspaceId, data, req.user?.sub);
    }

    @Patch('deals/:id')
    updateDeal(@Param('workspaceId') workspaceId: string, @Param('id') id: string, @Body() data: any, @Req() req: any) {
    ...
    @Delete('deals/:id')
    deleteDeal(@Param('workspaceId') workspaceId: string, @Param('id') id: string, @Req() req: any) {
        return this.kanbanService.deleteDeal(workspaceId, id, req.user?.sub);
    }

    @Patch('columns/:id')
    updateColumn(@Param('workspaceId') workspaceId: string, @Param('id') id: string, @Body() data: any, @Req() req: any) {
