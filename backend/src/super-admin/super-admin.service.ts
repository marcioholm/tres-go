...
export class SuperAdminService {
  ...
  async getDashboardMetrics() {
    ...
    for (const sub of activeSubs) {
      ...
      mrr +=
        sub.billingCycle === 'MONTHLY'
          ...
          : (sub.plan.priceYearly || 0) / 12;
    }

    const totalWorkspaces = await this.prisma.workspace.count();
    const blockedWorkspaces = await this.prisma.workspace.count({
    ...
    const ltv =
      churnRate > 0
        ? mrr / totalActive / (churnRate / 100)
        : (mrr / totalActive) * 24; // fallback 24 meses se churn 0
    ...
    return {
      ...
      blockedWorkspaces,
      newSignups: await this.prisma.workspace.count({
  ...
  private async getCriticalAlerts() {
    const alerts = [];
    const blocked = await this.prisma.workspace.findMany({
      where: { isBlocked: true },
      take: 5,
    });
    if (blocked.length > 0)
  ...
  async getSystemHealth() {
    ...
    return {
      ...
      services: {
        asaas:
          (await this.billing.getAsaasHealth()).status === 'connected'
            ? 'UP'
            : 'DOWN',
      },
    };
